...
export default function ReadingScreen() {
  ...
  const { getReadingById, updateReading } = useReadings();
  ...
  const translateIfNeeded = useCallback(async () => {
    if (!reading) return;
    const readingLang = reading.language ?? 'en';
    if (readingLang === language) return;

    setIsTranslating(true);
    try {
      const response = await fetch(`${API_BASE}/api/palm/translate-reading`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ analysis: reading.analysis, language }),
      });
      if (!response.ok) throw new Error('Translation failed');
      const { analysis: translated } = await response.json();
      await updateReading(reading.id, { analysis: translated, language });
    } catch (e) {
      console.error('Auto-translate error:', e);
    } finally {
      setIsTranslating(false);
    }
  }, [reading?.id, reading?.language, language]);
    ...
    return (
      <LinearGradient colors={['#0A0415', '#12082A']} style={styles.container}>
        <View style={[styles.centerWrap, { paddingTop: topPadding + 60 }]}>
          <Text style={styles.notFoundText}>{t('readingNotFound')}</Text>
          <Pressable onPress={() => router.back()} style={styles.backBtn2}>
            <Text style={styles.backBtn2Text}>{t('goBack')}</Text>
          </Pressable>
        </View>
      </LinearGradient>
    );
    ...
    <LinearGradient colors={['#0A0415', '#12082A']} style={styles.container}>
      {isTranslating && (
        <View style={styles.translateBanner}>
          ...
          <Text style={styles.translateBannerText}>{t('translating')}</Text>
        </View>
      )}
      <ScrollView
      ...
      >
          ...
          <LinearGradient colors={['#1A0D35', '#251350']} style={styles.overviewCardInner}>
            <View style={styles.overviewTop}>
              ...
              <View style={styles.overviewMeta}>
                ...
                {reading.dob && (
                  <Text style={styles.overviewDob}>{t('born')} {reading.dob}</Text>
                )}
              </View>
            </View>
        ...
        <Animated.View entering={FadeInDown.delay(350)} style={styles.section}>
          <Text style={styles.sectionTitle}>{t('palmLines')}</Text>
        ...
        <Animated.View entering={FadeInDown.delay(500)} style={styles.section}>
          <Text style={styles.sectionTitle}>{t('lifeInsights')}</Text>
          <View style={styles.linesList}>
            {insights.map((ins, i) => (
          ...
          <LinearGradient colors={['#1A0D35', '#251350']} style={styles.krishnaCard}>
            <View style={styles.krishnaCardTop}>
              ...
              <View style={styles.krishnaCardText}>
                <Text style={styles.krishnaCardTitle}>{t('haveDivineDoubts')}</Text>
                <Text style={styles.krishnaCardSub}>{t('krishnaInsightSub')}</Text>
              </View>
            </View>
            <Pressable
            ...
            >
              <LinearGradient colors={['#C9902A', '#E8B840']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.krishnaBtnGrad}>
                ...
                <Text style={styles.krishnaBtnText}>{t('askKrishna')}</Text>
...
const styles = StyleSheet.create({
  ...
  translateBanner: {
    ...
    backgroundColor: 'rgba(201,144,42,0.12)', paddingVertical: 10,
    borderBottomWidth: 1, borderBottomColor: 'rgba(201,144,42,0.2)',
  },
  translateBannerText: { fontSize: 13, fontFamily: 'Inter_500Medium', color: Colors.dark.gold },
