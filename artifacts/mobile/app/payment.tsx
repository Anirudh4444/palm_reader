...
const UPI_APPS = [
  {
    ...
    emoji: '💜',
    color: '#5F259F',
    upiId: '9938283488@ybl',
    scheme: `phonepe://pay?pa=9938283488@ybl&pn=${encodeURIComponent(NAME)}&am=${AMOUNT}&tn=${encodeURIComponent(NOTE)}&cu=INR`,
    webUrl: `upi://pay?pa=9938283488@ybl&pn=${encodeURIComponent(NAME)}&am=${AMOUNT}&tn=${encodeURIComponent(NOTE)}&cu=INR`,
  },
  {
    id: 'gpay',
    name: 'Google Pay',
    emoji: '🔵',
    color: '#4285F4',
    upiId: '9938283488@okicici',
    scheme: `tez://upi/pay?pa=9938283488@okicici&pn=${encodeURIComponent(NAME)}&am=${AMOUNT}&tn=${encodeURIComponent(NOTE)}&cu=INR`,
    webUrl: `upi://pay?pa=9938283488@okicici&pn=${encodeURIComponent(NAME)}&am=${AMOUNT}&tn=${encodeURIComponent(NOTE)}&cu=INR`,
  },
  {
    id: 'paytm',
    name: 'Paytm',
    emoji: '💙',
    color: '#00BAF2',
    upiId: '9938283488@paytm',
    scheme: `paytmmp://pay?pa=9938283488@paytm&pn=${encodeURIComponent(NAME)}&am=${AMOUNT}&tn=${encodeURIComponent(NOTE)}&cu=INR`,
    webUrl: `upi://pay?pa=9938283488@paytm&pn=${encodeURIComponent(NAME)}&am=${AMOUNT}&tn=${encodeURIComponent(NOTE)}&cu=INR`,
  },
];
...
export default function PaymentScreen() {
  ...
  const openUPIApp = async (app: typeof UPI_APPS[0]) => {
    ...
    try {
      ...
      if (canOpen) {
        ...
        if (canOpenGeneric) await Linking.openURL(app.webUrl);
      }
      setStep('pending');
    } catch {
      setStep('pending');
  ...
  const selectedUPIApp = UPI_APPS.find(a => a.id === selectedApp);
            ...
            {step === 'pending' ? (
              <View style={styles.pendingCard}>
                ...
                <Text style={styles.pendingMsg}>
                  Complete the ₹{AMOUNT} payment in your UPI app.{'\n'}UPI ID:{' '}
                  <Text style={styles.upiId}>{selectedUPIApp?.upiId ?? '9938283488@ybl'}</Text>
                </Text>
